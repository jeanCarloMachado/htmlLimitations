...
\limitation{jsSpecificationCycle}. Com a especificação pronta,
...
\limitation{jsImplementaionCycle}.
...
guardadas como texto (\limitation{webStorageStringOnly}). Isso
...
navegador \limitation{indexedDbAnonymousBehaviour}. Não é possível
realizar buscas em textos, algo similar ao \textit{LIKE} do SQL
\limitation{indexedDbNoLike}.
...
\Cref{limitation:jsImplementaionCycle} & Demora para implementar novas especificações do JavaScript & Não \\ \hline
...
\Cref{limitation:hardToUseWebGL} & Para construir jogos é muito difícil utilizar apenas WebGL & Sim \\ \hline
...
\Cref{limitation:limitedMultimidiaControlOnSafari} & O Safari conta com problemas de controle de execução de multimídia & \\ \hline
\Cref{limitation:noEffectsOnVideo} & Não é possível aplicar shaders diretamente em vídeo & Sim \\ \hline
\Cref{limitation:videoFullscreenControl} & Falta de forma padronizada de controlar o modo tela cheia para vídeos & \\ \hline
\Cref{limitation:videoCodecs} & Falta de consenso sobre um codec de vídeo padrão & \\ \hline
...
\Cref{limitation:webStorageStringOnly} & Web Storage permite que os dados sejam armazenados unicamente como Strings & Não \\ \hline
\Cref{limitation:indexedDbAnonymousBehaviour} & O comportamento do IndexedDB para abas anônimas varia & Não \\ \hline
\Cref{limitation:indexedDbNoLike} & Não é possível realizar buscas em texto com o IndexedDB & Não \\ \hline
\Cref{limitation:noErrorMessagesOffline} & Erros na configuração do manifesto offline não contém mensagens descritivas 0& Não \\ \hline
...
\Cref{limitation:noCanvas2DIsnpectorOnipresent} & Não existe inspetor Canvas 2D para navegadores multiplataforma & Parcial \\ \hline
...
\Cref{limitation:monetizationDifferent} & Jogos exclusivamente Web sentem dificuldade em monetizar jogos da forma convencional & Parcial \\ \hline
\caption{Lista de problemas e limitações}
\label{table:technologies}
\end{longtable}

Das limitaçõe dispostas no quadro \ref{table:technologies}
