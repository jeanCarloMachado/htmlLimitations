...
das tecnologias, outras dos dispostivos ou da característica
multiplataforma.

Abaixo constam as limitações encontradas durante a
pesquisa e concepção do jogo.
...
lançaram antes da especificação estar completa e hoje tem problema de
suporte para essas áreas \autocite{diveIntohtml}. Não obstante, canvas
está presente em todos os navegadores populares à partir do Internet
Explorer 9. Para navegadores antigos, abaixo do Internet Explorer 9,
existe o polyfill Explorer Canvas do Google, que emula em JavaScript as
funcionalidades do canvas.

Muitas das variadas implementações do canvas tem problemas de performance.
...
Por ser apenas sistema de mapa de pixels canvas carece de integração
com o restante das tecnologias da WEB. Não é possível aproveitar as
facilidades de animações do CSS, manipulação do DOM, entre outras
tecnologias.

Um fator descoberto durante o desenvolvimento do protótipo é que o
...
Canvas também não permite a manipulação das camadas já desenhadas, sendo 
necessário sobrescrever o canvas ou limpá-lo.
...
\subsubsection{Bibliotecas WebGL}
