...
força os desenvolvedores a suportarem ambas as versões. É razoável
...
casos, como o dos botões de certo e errado do protótipo, em que as
...
motores de renderização é um processo entediante e propenso a
...
6 contém suporte pobre na grande maioria dos motores, especialmente
quando referente a funcionalidade de subclasses.

Por ser uma linguagem interpretada, erros tem ser de descobertos rodando a
...
ofuscados. Muitas vezes esses processos precisam ser executados durante
...
problemas\textquotedbl{} (\limitation{runScriptsOnlyOnTheEndOfTheProcessment}). Pode-se
...
\textsc{\autocite{html5mostwanted} (\limitation{NANPropagation}}). Uma saída
para este problema é checar constantemente os tipos de forma a
assegurar-se que eles continuam válidos. No entanto, este processo
...
(\limitation{JSanimations}). O Web Assembly poderia resolver este problema,
...
muito complexos, onde posicionamento não seja um fator crucial.
...
separadamente do JavaScript. Devido as características acima citadas
...
som (\limitation{soundAPIConflicts}). A experimentação do
...
SQL para o lado do cliente (\limitation{noSqlSupport}). O WebSQL foi uma
...
\textsc{\autocite{gameAssetManagement} (\limitation{webStorageLimit}}). Também
...
\textsc{\autocite{gamepad} (\limitation{gamepadObject}}). Este problema é
...
\Cref{limitation:jsImplementaionCycle} & O suporte às novas especificações muitas vezes demora para ser implementado & Sim \\ \hline
...
do tempo. Como no caso da limitação \Cref{limitation:gamepadObject},
...
(\Cref{limitation:fullscreenManagement}). Em plataformas nativas esse
