...
A parte denominada: "Hyper Text", refere-se a links que conectam páginas umas as
outras, fazendo a Web como conhecemos hoje \cite{mdn2015}.

HTML foi especificado baseado-se em SGML (Standard Generalized Markup Language). Diferentemente de XML, HTML não pode ser decodificado através de geradores de tokens (\textit{tokenizers}) convencionais. 

A impossibilidade de tradução via métodos convencionais, deve-se ao fato do HTML ser amigável com o programador, aceitando erros de sintaxe, dependente de contexto, buscando entregar a melhor aproximação possível. Essa limitação deu origem uma especificação própria de como renderizar HTML.
...
Antes de HTML5 várias versões foram propostas, algumas radicais em seus conceitos, como o XHTML4.

A maioria das versões HTML de grande sucesso foram versões de retrospectiva (\textit{"retro-specs"}), entrando em sincronia com o mundo enquanto busca a direção correta \autocite{diveIntohtml}.
...
Existem mais de cem elementos no HTML5 \autocite{diveIntohtml}.
...
A tag \textit{canvas} define um layer gráfico em documentos HTML que pode
...
{\thispagestyle{myheadings}\printbibliography}
