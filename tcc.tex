...
web. Criada por Tim Berners Lee em 1989 e oficialmente mantida pela W3C.
...
a WHATWG decidiu renomear o HTML5 para HTML \autocite{htmlIsTheNewHtml5}. Não obstante, o termo HTML5
permanece em grande utilização.

Antes de HTML5 várias versões foram propostas, algumas radicais em
seus conceitos, como o XHTML4; o qual quebrava praticamente toda a compatibilidade
com as versões anteriores do HTML.

A maioria das versões HTML de grande sucesso foram versões de
retrospectiva (\textit{"retro-specs"}). Versões que não tentavam idealizar a linguagem
entrando em sincronia com os requerimentos do mercado enquanto em busca da direção correta \autocite{diveIntohtml}.

Diferentemente de XML, HTML não pode ser decodificado
através de \textit{tokenizers} convencionais.  Isso deve-se
...
dele reside \autocite{htmlAndCssDucket}.%pg 20

O HTML5 introduziu elementos interativos que
viabilizaram a construção de jogos para a plataforma como: Canvas,
áudio, vídeo.

Na versão inicial o HTML contava com 18 elementos.
Atualmente existem mais de cem elementos no HTML5 \autocite{diveIntohtml}.
...
suportados pelos navegadores é o site http://caniuse.com/.
...
ser desenhada através de JavaScript.

O canvas é um retângulo em uma página web onde pode-se usar JavaScript para desenhar qualquer coisa \autocite{diveIntohtml}.

A API canvas cresceu com o tempo, e algumas funciondades (como texto), foram adicionadas tardiamente. E alguns navegadores lançaram antes da especificação estar completa \autocite{diveIntohtml}.


Permite desenhar diagramas, gráficos e animações [7]. É baseado em
bitmap.

Muitas vezes lento. Algumas soluções híbridas buscam arrumar isso através da
