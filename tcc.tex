...
\begin{itemize}
    ...
    \item  São portáveis devido a definição de tipo de documento (\textit{document type declaration});
\end{itemize}
...
a compatibilidade com as versões anteriores do HTML. Não obstante,
a maioria das versões HTML de grande sucesso foram versões de
retrospectiva (\textit{"retro-specs"}). Versões que não tentavam
idealizar a linguagem, buscando sincronizar-se com os requerimentos do
mercado \autocite{diveIntohtml}.

Diferentemente de XML, HTML não pode ser decodificado
através de tokenização tradicional. Deve-se
ao HTML ser amigável ao programador, aceitando erros
de sintaxe, dependente de contexto, buscando entregar a melhor
aproximação possível. Esta característica deu origem a uma especificação
para renderizar HTML (\textit{HTML parser}).
...
dele reside \autocite[pp. 10--11]{htmlAndCssDucket}. Na figura \ref{fig:htmlSample}
é demonstrado vários exemplos de tags, como a tag \textit{body}.
