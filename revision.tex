...
formam a Web o que leva a comportamentos inesperados em alguns caos,
...
a links que conectam páginas HTML umas às outras, fazendo a Web como
...
propostos e seu estágio de evolução. O quadro \ref{table:ecma7} é
um resumo das funcionalidades sugeridas e seu estágio no processo da
...
\begin{figure}[H]
    \centering
    \includegraphics[width=16cm]{ecma7.png}
\end{figure}
...
\begin{lstlisting}[language=html,frame=single]
	...
	request.onsuccess = function(event) {
		...
		var request = objectStore.add(
            {email: "mymail@domain.com", name: "foo"}
        );
		request.onsuccess = function(event) {
...
imprescindível grande flexibilidade na gestão de entrada de comandos.
...
teclado, em dispositivos móveis através de tela sensível ou sensor de
...
ocorre seja um clique do mouse ou o pressionar de uma tecla ou o mover de
...
qualquer controle desejado \textsc{\autocite{buildingHtml5Game}}.
O teclado é um periférico comum no caso de jogos da Web. Para
manipular as teclas existem os eventos \textit{keyup} e
\textit{keydown} que representam uma tecla sendo solta e pressionada
respectivamente. A \ref{fig:keyboardEvents} demostra a captura do
pressionar das setas em JavaScript. Cada número corresponde a um botão
do teclado especificado através da tabela ASCII.
...
através de software de terceiros. E sua utilização é limitada,
restringida à emulação de mouse e teclado, subutilizando seus
...
Exitem eventos para atividades esporádicas como a conexão e
...
de um servidor Web esta requisição é geralmente feita através
do protocolo HTTP. O foco da nova versão do HTTP é performance;
especialmente a latência percebida pelos usuários e o uso de rede e
servidores \textsc{\autocite{http2}}. A forma que os documentos trafegam
do servidor para o cliente afeta diretamente a performance de uma
aplicação, nos jogos esse fator se amplia devido a grande quantidade
de arquivos geralmente necessários para montar uma cena de jogo.
...
Dentro do navegador as requisições HTTP/2 são convertidas em
...
do Google Chrome do computador e utilizar as já mencionadas tecnologias do
DevTools.
...
foi removido e atualmente inspeção de canvas é um terreno escasso
...
ferramenta de minificação capaz de gerar Source Maps. Após o jogo ter
sido depurado e seus erros minimizados, pode-se disponibilizar para ser
consumido por seus usuários finais.
