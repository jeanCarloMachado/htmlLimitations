...
Armazenamento local é um recurso importante para jogos,tanto por diminuir a latência
da persistência que na rede pode ser grande, ou para possibilitar um experiência off-line.

Existem várias especificações sobre armazenamento, mas a grande maioria delas não conta como
suporte completo em todos os navegadores comuns, um polyfill interessante para localstorage
IndexedDB e WebSQL é o projeto localForge https://github.com/mozilla/localForage da mozilla.
...
quantidades de dados no formato de chave/valor.
Assim como SQL, todas as mudanças no IndexedDB ocorrem através de
transações.
...
interesse da comunidade. Por assíncrona significa que aplicativos
JavaScript nunca esperam pelo final das transações para continuar funcionando,
outrossim, cada interação com o banco de dados é uma requisição que pode retornar um
resultado ou um erro. Um evento de sucesso internamente é um evento DOM cuja propriedade
\textit{type} do elemento foi setada para \textit{success}.

Ao invés de tabelas, IndexedDB trabalha com repositórios de objetos. Cada objeto armazenado
conta com ao menos uma chave para referenciá-lo.
...
Apesar de ser desenvolvido em o objeto de ser uma solução para todas
as necessidades de armazenamento no frontend IndexedDB ainda sofre
alguns problemas.
Os limites de armazenamento e estratégias de limpeza variam de
navegador para navegador.

\begin{itemize}
\item Indexeddb tem limites de armazenamento e as regras variam de navegador para navegador.
\item O comportamento em abas anônimas não está especificado e os resultados também variam
\item Existe uma pequena probabilidade de os dados corromperem, essa foi uma escolha em detrimento de performance.
\item Não existe a possibilidade de fazer buscas em textos como o like do sql
\item o usuário pode configurar o navegador para não aceitar armazenamnto local para determinado domínio
\end{itemize}

Essa característica assíncrona do IndexedDB é interessante especialmente no contexto de Web Workers.
