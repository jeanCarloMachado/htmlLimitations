...
uma gama crescente de estudos apontando os benefícios dos mesmos.

Video games melhoram as funções cognitivas, as capacidades criativas, e
...
interesse humano. Segundo \autocite{gamebenefits} habilidades espaciais
estão diretamente relacionadas com o sucesso em ciência, tecnologia,
engenharia e matemática.

\subsection{GÊNEROS}

Uma classificação de jogos em gêneros é uma tarefa complexa.
...
Os primeiros jogos WEB eram limitados a tecnologias da época. Um gênero de
que se desenvolveu bem foram os jogos de estratégia em texto como o Travian.

Com as inovações do HTML5 novas possibilidades e gêneros de jogos
podem ser explorados na WEB. Cada gênero acompanha um conjunto de
desafios específicos Jogos de FPS requerem baixos níveis de          .
ping, já jogos de RPG podem requerer vastas quantidades de cache      .
...
desenvolvimento no caso dos jogos. Ao criar jogos deve-se planejar o
que se pretende atingir e como chegar lá antes de se escrever qualquer
código.

O sumário também conta com um resumo sobre metodologias de desenvolvimento de jogos.

Se os gráficos e áudio são espetaculares mas a jogabilidade é chata
o jogador vai parar de jogar. A substância do jogo é sua mecânica,
então não invista muito em visual ao menos que isso desempenhe um
papel essencial no jogo \autocite{html5mostwanted}.

\section{JOGOS MULTIPLATAFORMA}

Jogos em plataformas móveis trazem um novo conjunto de desafios
...
quando em face de desenvolver um novo jogo: Criar um jogo web, um jogo
...
torna amplamente popular, através da adoção de grandes empresas e]nc
desenvolvedores, ela se torna candidata a adoção pela OWP. 
As tecnologias da Open WEB tem evoluído desde os princípios da internet e já provaram sua robusteza e estabilidade enquanto outras tecnologias crescem e morrem ao redor dela \autocite{html5mostwanted}.
...
antes necessária para estilizar uma página. Segundo \cite{html5mostwanted} pode-se expressar regras gerais que são "cascateadas" para muitos elementos, e então sobrescrever os elementos específicos conforme a necessidade.

A diferenciação entre apresentação e estrutura, sendo neste caso
o CSS responsável pela apresentação, é um dos pontos chave do
SGML, motivo que tornou a utilização do CSS tão conveniente para o
desenvolvimento WEB.
...
Carregar regras condicionalmente permite implementar fluidez e
...
sobre seus estados. Habilitando o desenvolvedor a controlar
o tempo de execução, os estados intermediários, e efeitos aplicados uma transição.

Para utilizar transições, como em uma máquina de estados, precisamos identificar estados e ações. Estados são seletores do CSS e ações são modificações  realizadas entre esses dois seletores CSS \autocite{html5mostwanted}.
...
Segundo \cite{html5mostwanted} transições nos permitem construir jogos degradáveis pois os interpretadores de CSS são amigáveis; se eles encontrarem propriedades desconhecidas eles simplesmente as ignoram e continuam a funcionar.
...
Permitem que elementos sejam traduzidos, rotacionados, escalados e distorcidos em um espaço de duas dimensões \autocite{html5mostwanted}.
...
utilizados para 2D também \autocite[pp. 6]{3daps}. 
...
especifica o OpenGL: Kronos Working Group.Os primeiros
