...
%{{{
\begin{draft}
Disponibilizar aplicações WEB offline é uma possibilidade introduzida
do HTML5. Para tanto é necessário poder armazenar dados
locais, tecnologias como IndexedDB e Web Storage permitem isso. Outro
...
Para tanto HTML5 provê uma especificação de APIs de cache offline.
A especificação determina que uma arquivo de manifesto contenha 
quais arquivos serão guardados para utilização offline e possivelmente 
quais serão usados pela rede.

A figura \ref{fig:off-line} exemplifica um arquivo de manifesto. 
Os arquivos abaixo da palavra \textit{CACHE MANIFEST} serão armazenados
em cache para utilização offline. Já os arquivos abaixo da palavra \textit{NETWORK:}
serão exclusivamente da rede.
...
\textit{listener} pode invocar uma função e realizar qualquer coisa desejada
