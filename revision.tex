...
a linha deve percorrer. Além da possibilidade de desenhar linhas
livremente também é possível retângulos nativamente, todas as demais figuras geométricas precisam ser feitas através da junção de linhas \autocite{mdnCanvas}.

Além da possibilidade de desenhar programaticamente é possível
carregar gráficos. A maioria dos jogos HTML5 utilizam sprites ou
padrões recortáveis \textit{tiled}, muito similar aos títulos antigos
da SNES e Game Boy \autocite{buildingHtml5Game}.

Não é possível manipular diretamente as camadas já desenhadas do canvas. As alternativas são limpar o canvas em determinada área ou sobrescrevê-la.

Segundo \cite{gwt} uma característica importante deste [canvas] elemento é que ele consegue redimensionar seu conteúdo para adequar à resolução do navegador do usuário.
