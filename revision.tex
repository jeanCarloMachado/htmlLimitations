...
Codec é o algoritmo usado para codificar e decodificar o vídeo ou
áudio em um conjunto de bits \cite{diveIntohtml}. O termo Codec
é um acrônimo, significando codificar/decodificar. O objetivo
dos codecs é diminuir o tamanho dos arquivos com a menor perda de
qualidade possível, para isso eles utilizam de várias estratégias de
compressão ou descarte de dados.

Depois de comprimido, o fluxo de dados é armazenado em um contêiner,
que é um conjunto de metadados sobre as informações codificadas
modo a possibilitar que outros programas consigam interpretar estas
informações de forma padronizada.

O funcionamento dos codecs e contêiner varia de produto para produto.
A complexidade do funcionamento de codecs varia conforme sua qualidade
e taxa de compressão. Visto que os algoritmos podem adquirir grande
complexidade, muitos codecs e contêiner são encobertos por licenças
que limitam sua utilização. Não obstante, também existem opções
livres de patentes e licenças.

O suporte a codecs na WEB varia de navegador para navegador, de acordo
com as preferências mercadológicas ou filosofias das empresas. Segundo \cite{diveIntohtml} não existe
uma única combinação de contêiner e codecs que funcionem em todos os
navegadores.
...
som e música podem servir como mecanismo. Por outro lado, jogadores tem
baixa tolerância a volume, deve ser utilizado com cautela.
...
pode ser reproduzido pelo player de audio do navegador.

A especificação declara que todo o conteúdo dentro de uma tag áudio
(e vídeo também), que não sejam tags source, sejam ignoradas pelo navegador.
O que permite que, quando audio não é suportado pelo navegador conteúdo 
extra seja mostrado.
