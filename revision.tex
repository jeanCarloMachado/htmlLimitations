...
\section{WEBGL}
%{{{
WebGL é uma API JavaScript para desenhar elementos 3D. É Baseado
em OpenGL, mas especificamente em OpenGL ES 2.0, uma versão do OpenGL
otimizada para dispositivos móveis. O órgão que especifica o WebGL é
o mesmo que especifica o OpenGL: Kronos.

Os primeiros rascunhos do WebGL iniciaram em 2006, não obstante o grupo
de trabalho não foi formado até 2009.
A primeira versão do WebGL foi lançada em 2011.

A especificação é composta por uma API de controle em JavaScript e o
shaders que são executados na GPU. O elemento que provê a interface 
do WebGL é o canvas, no contexto 3D.

Apesar de ter sido desenvolvido para gráficos 3D, WebGL pode ser utilizados
para 2D também \autocite{}.
...
São utilizados no cinema, em imagens geradas por computadores e video games.
